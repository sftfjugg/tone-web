...
class DistributedBackgroundScheduler(BackgroundScheduler):
    ...
    def _process_jobs(self):  # noqa: C901
        """
        ...
        if self.state == STATE_PAUSED:
            ...
            return None
        identifier = redis_cache.acquire_lock('single_scheduler_lock', acquire_time=600, time_out=600)
        if not identifier:
            return None
        ...
        if self.state == STATE_PAUSED:
            ...
            self._logger.debug('Next wakeup is due at %s (in %f seconds)', next_wakeup_time,
                               wait_seconds)
        redis_cache.release_lock('single_scheduler_lock', identifier)
        return wait_seconds
